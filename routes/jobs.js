...
router.post('/getalljobs' , async(req , res) =>{
  try {
    var jobs = await Job.find().pretty();
    var userId = req.body.userId;
    jobs.map(async job =>{
      const likers = await Job.find ({_id: job._id},{likers:1,_id:0}).pretty();
      const dislikers = await Job.find({_id: job._id},{dislikers:1,_id:0}).pretty();
      var status = 0;
      ...
      job = await Job.find({_id: req.body.jobId}).pretty();
      job.status = status;
    })
    res.send(job);
  } catch (error) {
...
router.post('/getonejob', async(req , res) => {
    try{     
      ...
      var status = 0;
      for(var i = 0; i < likers.length; i++){
        if(likes[i]==req.body.userId){
          status = 1;
        }
      }
      for(var i = 0; i <dislikers.length; i++){
        if(dislikes[i].id==req.body.userId){
          status = 2;
        }
      }
      res.send(job);
